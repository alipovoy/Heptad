...
- **Single Window Architecture**: Uses a dynamically reconfigurable window transitioning between two states:
  - **Attached State**: Opens anchored below the status bar icon. Behaves as a non-activating utility window (no Dock icon, no main menu). Auto-dismisses when clicking outside.
